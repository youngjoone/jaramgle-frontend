...
export default function Home() {
  ...
  const [viewingStorybook, setViewingStorybook] = useState<Storybook | null>(null);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="md:ml-20 pb-20 md:pb-0">
        {activeNav === 'create' && <CreatePage />}
        {activeNav === 'explore' && (
          <ExplorePage
            storybooks={storybooks.filter(book => book.isShared)}
            onToggleBookmark={(id) => {
              setStorybooks(prev =>
                prev.map(book =>
            ...
            onViewStorybook={(storybook) => setViewingStorybook(storybook)}
          />
        )}
        {activeNav === 'subscription' && <CreditPurchasePage />}
        {activeNav === 'gallery' && (
          <GalleryPage
            storybooks={storybooks.filter(book => book.isOwned)}
            onToggleShare={(id) => {
              setStorybooks(prev =>
                prev.map(book =>
        ...
        {activeNav === 'profile' && <ProfilePage onLogout={() => setIsLoggedIn(false)} />}
      </div>
